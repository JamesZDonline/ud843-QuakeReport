...
public final class QueryUtils {
    ...
    private static String makeHttpRequest(URL url) throws IOException{
        ...
        //If url is null return an empty string
        if(url == null){
            return jsonResponse;
        }

        //Initialize connection and input streams
        HttpURLConnection urlConnection = null;
        InputStream inputStream = null;
        //tryto open the connection
        try {
            ...
            //if successful response parse the response
            if (urlConnection.getResponseCode() == 200) {
                inputStream = urlConnection.getInputStream();/*get the input stream from the connection*/
                jsonResponse = readFromStream(inputStream);/*pass to method to read the stream */
